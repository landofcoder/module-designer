...
if(jQuery(".designerowl-play").length > 0 ) {
	var owlItems = [];
	jQuery(".designerowl-play").each( function(){
		...
		if(!owlItems[jQuery(owlCarousel).attr("id")]){
			...
			if(jQuery("#" + id + " .designer-item").height){
				var height = 0;
				jQuery("#" + id + " .designer-item").each(function(){
					if(jQuery(this).height()>height){
						height = jQuery(this).height();
					}
				});
				if(height>0){
					jQuery("#" + id + " .designer-item").css({"height":height+"px"});
				}
			};
		};
	});
